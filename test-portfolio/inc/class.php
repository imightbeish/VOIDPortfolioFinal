...
class Onepress_Customize_Repeatable_Control extends WP_Customize_Control {
	...
	public function render_content() {
		...
		<div class="repeatable-actions">
			<span class="button-secondary add-new-repeat-item"><?php echo ( $this->add_text ) ? esc_html( $this->add_text ) : __( 'Add a item'); ?></span>
		</div>
		<?php
	}
								...
								<div data-field-id="{{ field.id }}"  class="field--item item item-{{ field.type }} item-{{ field.id }}" >
									...
									<select data-live-id="{{ field.id }}"  class="select-multiple" multiple="multiple" data-repeat-name="_items[__i__][{{ field.id }}][]">
										...
										<div class="actions">
											...
											<button class="button upload-button" data-media="{{field.media}}" data-add-txt="<?php esc_attr_e( 'Add'); ?>" data-change-txt="<?php esc_attr_e( 'Change'); ?>" type="button"><# if ( ! field.value.url  ){ #> <?php _e( 'Add'); ?> <# } else { #> <?php _e( 'Change'); ?> <# } #> </button>
